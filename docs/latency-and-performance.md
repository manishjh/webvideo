...
Render-path labels are part of the production diagnostics. Direct single-canvas playback and the shared VMS matrix normally report `external-texture / webgpu-canvas` on the hardware path. The offscreen VMS matrix uses `matrixRetain=auto`: a one-slot matrix or a full-redraw pass renders directly into the current swapchain texture with viewport/scissor, while partial multi-tile updates use a persistent WebGPU backing texture so unchanged tiles stay visible without relying on swapchain persistence. This avoids the intermediate per-frame `VideoFrame` -> `GPUTexture` copy. `?matrixRetain=swapchain` and `?matrixRetain=backing` are profiling switches, and `?matrixTexture=copy` keeps the older retained `GPUTexture` upload path available for fallback comparison.
...
- Duplicate views share one WebTransport/WebCodecs session per channel. The matrix compositor ref-counts retained `VideoFrame` objects so duplicate views can redraw the same decoded frame through external textures without cloning or copying it into separate retained source textures.
- Matrix present path is now measured separately from tile render. The profile JSON includes matrix present path, flush/present/draw/import/bind-group counters, and per-stage render timing; use those counters to distinguish decode/network backlog, backing-texture copy cost, and `VideoFrame` import stalls.
- Full-resolution 4K60 remains a browser import bottleneck on the local Chrome/Linux path. The latest single-stream profiles receive and decode 4K60 continuously, but `importExternalTexture` p95 is still tens of milliseconds, while bind/uniform/encode/submit are sub-millisecond. That means shader-level OSD and command encoding are not the current limiter.
...
That points the next optimization at proving the cheapest browser-supported path from WebCodecs `VideoFrame` to presentation, fewer per-frame WebGPU API calls, page/session-level transport multiplexing, a real source-control path for non-destructive variant switching, tighter worker handoff policy, and reducing startup warm-up hitches. Workerizing transport/MoQ parse and decode orchestration is now measurable with `?mediaWorker=1`. `?predecodeFrameAdmission=1` is available for profiling safe AVC non-reference-frame shedding, but it is not default because dropping arbitrary delta frames before decode is unsafe for H.264 prediction. WASM decode is not a preferred fast path while hardware WebCodecs is working; it would be a fallback or parser utility, not the way to beat native H.264 decode.
