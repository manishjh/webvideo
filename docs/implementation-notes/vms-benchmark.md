...
| Shape | Result | Notes |
|---|---|---|
| Single full-resolution 4K60 crowd, per-tile offscreen worker, `video-strict-vulkan` Chrome preset | Diagnostic pass, not product-grade | Profile `.run/profiles/vms-profile-1streams-1781600813963.json` received about 59.6 fps from WebTransport and decoded with WebCodecs at about 1.3 ms p95, but rendered only about 14.0 fps steady. Render p95 was about 45.8 ms and `importExternalTexture` p95 was about 45.4 ms; bind, uniform, encode, and submit were all sub-millisecond. This isolates the current 4K60 blocker to browser/driver `VideoFrame` -> WebGPU external texture import, not app shader work. |
| Single full-resolution 4K60 crowd, shared offscreen matrix auto-retain | Diagnostic pass, not product-grade | Profile `.run/profiles/vms-profile-1streams-1781599859939.json` used `matrixPresentPath=swapchain`, avoiding the retained backing-texture copy for the one-slot case. It still rendered only about 13.6 fps steady, with render p95 about 57.6 ms and import p95 about 57.0 ms. Removing the viewport copy was not enough; external texture import remained dominant. |
| Single full-resolution 4K60 crowd, Intel/QuickSync-flavored low-power WebGPU adapter | Diagnostic pass, worse than NVIDIA WebGPU path | Profile `.run/profiles/vms-profile-1streams-1781600243887.json` selected `intel gen-9`, but steady import p95 was about 49.6 ms and render p95 about 51.3 ms. On this machine the NVIDIA WebGPU adapter remains the better tested renderer for this workload. |
...
The dominant frontend CPU-profile signatures under stress are browser-native WebGPU calls (`importExternalTexture`, `copyExternalImageToTexture` when forced, `createBindGroup`, `getCurrentTexture`, `queue.submit`) plus smaller transport parse and matrix flush costs. The offscreen matrix compositor now uses `matrixRetain=auto`: one-slot and full-redraw presents render directly to the swapchain with viewport/scissor, while partial multi-tile updates use the retained backing texture so unchanged tiles stay visible without relying on swapchain persistence. Matrix profile JSON now reports present path, flushes, presents, draws, external imports, bind groups, VideoFrame copy counts, and visible-frame hash changes, which makes present starvation, stale overlays, and per-frame API pressure visible. The next serious optimization work should focus on reducing external-texture/import pressure, validating whether Chrome is producing hardware-backed WebCodecs frames for our H.264 streams, preventing 4K `VideoFrame` back-resource import failures, in-session source-control protocol work, and eventual page/session-level WebTransport multiplexing.

`WEBVIDEO_CHROME_WEBGPU_PRESET=video-strict-vulkan` adds Linux accelerated video-decode feature flags to the known-good Vulkan WebGPU launch shape. It improved one short NVIDIA run from roughly 46 ms import p95 to roughly 27 ms import p95, and reduced decode p95 to about 0.5 ms, but later runs still showed roughly 45 ms import p95. Treat it as a profiling preset, not a proven production fix.

`?predecodeFrameAdmission=1` enables an experimental media-worker gate that may drop only AVC Annex-B non-reference VCL frames before WebCodecs when render/import service time is over budget. It is intentionally opt-in because dropping arbitrary delta frames would corrupt prediction chains, and the current 4K60 crowd clip did not show a throughput win from the safe non-reference-only policy.
